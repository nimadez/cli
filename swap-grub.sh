#!/bin/bash
#
# Configuring GRUB to enable hibernate/suspend from a swap file (for debian command-line)
#
# Notice: We assume that GRUB_CMDLINE_LINUX= is empty
# Test hibernation: systemctl hibernate (required package: polkitd)
...
# update GRUB configuration with resume parameters
